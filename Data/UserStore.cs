...
{
    public class UserStore : IUserStore<AppUser>, IUserEmailStore<AppUser>, IUserPhoneNumberStore<AppUser>, IUserPasswordStore<AppUser>, IUserTwoFactorStore<AppUser>, IUserRoleStore<AppUser>
    {
        ...
        public async Task<IdentityResult> CreateAsync(AppUser user, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                //await connection.QuerySingleAsync($@"INSERT INTO [AspNetUsers] ([UserName],[Email],[NormalizedUserName],[NormalizedEmail],[EmailConfirmed],[PasswordHash],[PhoneNumber],[PhoneNumberConfirmed],[TwoFactorEnabled],[FullName],[Address],[LockoutEnabled],[AccessFailedCount])
                //     VALUES(@{nameof(AppUser.Id)}, @{nameof(AppUser.UserName)},@{nameof(AppUser.NormalizedUserName)},@{nameof(AppUser.Email)},@{nameof(AppUser.NormalizedEmail)},@{nameof(AppUser.EmailConfirmed)},@{nameof(AppUser.PasswordHash)},@{nameof(AppUser.PhoneNumber)},@{nameof(AppUser.PhoneNumberConfirmed)},@{nameof(AppUser.TwoFactorEnabled)},@{nameof(AppUser.LockoutEnabled)},@{nameof(AppUser.AccessFailedCount)});", user);
            }
        ...
        public async Task<IdentityResult> UpdateAsync(AppUser user, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                await connection.ExecuteAsync($@"UPDATE [AspNetUsers] SET 
                 ...
                 [AccessFailedCount] =@{nameof(AppUser.AccessFailedCount)}
        ...
        public async Task AddToRoleAsync(AppUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var roleId = await connection.ExecuteScalarAsync<Guid?>($"SELECT [Id] FROM [AspNetRoles] WHERE [NormalizedName]=@{nameof(normalizedName)}");
                if (!roleId.HasValue)
                {
                    roleId = Guid.NewGuid();
                    await connection.ExecuteAsync($"INSERT INTO [AspNetRoles]([Id],[Name],[NormalizedName]) VALUES(@{nameof(roleId)},@{nameof(roleName)}, @{nameof(normalizedName)})", new { roleName, normalizedName });
                }
                await connection.ExecuteAsync($"IF NOT EXISTS(SELECT 1 FROM [AspNetUserRoles] WHERE [UserId]=@userId AND [RoleId]=@{nameof(roleId)})" + $"INSERT INTO [AspNetUserRoles]([UserId], [RoleId]) VALUES(@userId, @{nameof(roleId)})",
        ...
        public async Task RemoveFromRoleAsync(AppUser user, string roleName, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                await connection.OpenAsync(cancellationToken);
                var normalizedName = roleName.ToUpper();
                var roleId = await connection.ExecuteScalarAsync<int?>($"SELECT [Id] FROM [AspNetRoles] WHERE [NormalizedName]=@{nameof(normalizedName)}");
                if (!roleId.HasValue)
        ...
        public async Task<IList<string>> GetRolesAsync(AppUser user, CancellationToken cancellationToken)
        {
            ...
            //{
            //    await connection.OpenAsync(cancellationToken);
            //    var queryResults = await connection.QueryAsync<String>("SELECT r.[Name] FROM [AspNetRoles] r INNER JOIN [AspNetUserRoles] ur ON ur.[RoleId] = r.Id" + "WHERE ur.UserId = @userId", new { userId = user.Id });
            //    return queryResults.ToList();
            //}
        }
        ...
        public async Task<IList<AppUser>> GetUsersInRoleAsync(string roleName, CancellationToken cancellationToken)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                var queryResults = await connection.QueryAsync<AppUser>("SELECT u.* FROM [AspNetUsers] u" + "INNER JOIN [AspNetUserRoles] ur ON ur.[UserId]=u.Id INNER JOIN [AspNetRoles] r ON r.[Id] = ur.[RoleId] WHERE r.[NormalizedName]=@normalizedName", new
                {
